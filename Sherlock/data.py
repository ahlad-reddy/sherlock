...
def build_food101_dataset(split="train", image_shape=(224, 224), rotate=False, batch_size=16, epochs=None, shuffle=True):
	...
	if shuffle:
		ds = ds.shuffle(2048)
	ds = ds.batch(batch_size).repeat(epochs).prefetch(tf.data.experimental.AUTOTUNE)
	if rotate:
		ds = ds.map(_rebatch)
